...
logging.basicConfig(filename='chatgpt.log', filemode='w', level=logging.INFO ,format='%(name)s - %(levelname)s - %(message)s')
model = {"davinci": "text-davinch-003", "gpt3.5": "gpt-3.5-turbo", "babbage": "babbage-002", "gpt4": "gpt-4"}
...
# 인공지능의 응답 객체 반환
def get_response_object(message, model=model["gpt3.5"], user='user'): 
...
# 응답 객체의 메세지 내용
def get_response(message, user='user'):
    logging.info('returning message')
    return get_response_object(message, user=user).choices[0].message.content
    # try: 
